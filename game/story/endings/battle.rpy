...
label cmdinput:
    ...
    if cmd == "help": # Help commands
        ...
        $ preferences.afm_time = 7
        "APERTURE SCIENCE CORE CONTROL CENTER (ASC3)\n[[ YIELD ] COMMAND\n\nYIELD ATTEMPTS A HACK INTO MAIN CORE SYSTEMS. 75 PERCENT\nCHANCE OF STUNNING CORE FOR ONE (1) CYCLE.\nUSES ONE (1) ACTION POINT."
        jump cmdinput
    elif cmd == "help delete":
        $ preferences.afm_time = 7
        "APERTURE SCIENCE CORE CONTROL CENTER (ASC3)\n[[ DELETE ] COMMAND\n\nTHIS FUNCTION MAY CAUSE CRITICAL DAMAGE TO CORE. USE WITH CAUTION. MAIN SYSTEMS PROTECTED. CONTACT ENCODING FOR\nADDITIONAL RE-ADJUSTMENT IF NECESSARY.\nUSES TWO (2) ACTION POINTS."
        jump cmdinput
    elif cmd == "help print":
        $ preferences.afm_time = 7
        "APERTURE SCIENCE CORE CONTROL CENTER (ASC3)\n[[ PRINT ] COMMAND\n\nATTEMPTS TO \"SPEAK\" TO THE CORE VIA A CODE STRING. CORE MUST HAVE AN OPEN CHANNEL FOR THIS COMMAND TO FUNCTION PROPERLY. USES ONE (1) ACTION POINT."
        jump cmdinput
    elif cmd == "help return":
        $ preferences.afm_time = 7
        "APERTURE SCIENCE CORE CONTROL CENTER (ASC3)\n[[ RETURN ] COMMAND\n\nPASSES COMMAND INPUT AND ENDS CYCLE, ALLOWING CORE TO EXECUTE COMMANDS. ADDS ONE (1) ACTION POINT."
        jump cmdinput
        ...
        if ap_count == 2:
            ...
            "NOT ENOUGH ACTION POINTS REMAINING. UNABLE TO EXECUTE."
            jump cmdinput
        ...
        if ap_count >= 1 and esther_stunned == False:
            ...
            if ap_count == 0:
                "NO MORE ACTION POINTS REMAINING. UNABLE TO EXECUTE."
                jump cmdinput
